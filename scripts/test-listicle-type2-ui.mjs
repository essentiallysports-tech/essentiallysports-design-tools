...
assert.match(source, /function drawListicleType2Rank[\s\S]*?const widthReference = isSingleDigit \? '3' : value;/, 'all single-digit badges must use the number 3 as their common width reference');
assert.match(source, /function drawListicleType2Rank[\s\S]*?const pillWidth = Math\.min\(width, Math\.ceil\(referenceGlyphWidth \+ horizontalPad \* 2\)\);[\s\S]*?const visibleLeft = x \+ \(pillWidth - glyphWidth\) \/ 2;/, 'rank badges must share one width and optically center each visible numeral');
